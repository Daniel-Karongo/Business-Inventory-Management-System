...
} from '../../../../shared/components/device-audit-dialog/device-audit-dialog.component';
...
export class PlatformDevicesComponent
    ...
    computeStats() {
        ...
        this.stats.approvedDevices =
            this.devices.filter(
                x => x.status === 'APPROVED'
            ).length;

        this.stats.pendingDevices =
            this.devices.filter(
                x => x.status === 'PENDING'
            ).length;

        this.stats.rejectedDevices =
            this.devices.filter(
                x => x.status === 'REJECTED'
            ).length;

        this.stats.devicesInUse =
            this.devices.filter(
                d => (d.usedByUsernames || []).length > 0).length;
    }
