...
export class BreadcrumbComponent implements OnInit {
  ...
  buildBreadcrumb() {
    ...
    let currentRoute: ActivatedRoute | null =
      this.route.root;
    ...
    while (currentRoute !== null) {
      ...
      currentRoute.snapshot.url.forEach(seg => {
        ...
        // Preserve full URL accumulation
        url.push(seg.path);

        // Hide technical shell segment from display
        if (seg.path === 'app') {
          return;
        }

        // Use param value as label when segment is parameterized
        const paramKey =
