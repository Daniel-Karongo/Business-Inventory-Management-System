...
public enum ReportAccess {
    ...
    CUSTOMER_OUTSTANDING_BALANCES("customer_outstanding_balances", Role.MANAGER),

    CUSTOMER_SALES_HISTORY_SINGLE("customer_sales_history_single", Role.SUPERVISOR),
    CUSTOMER_OUTSTANDING_BALANCE_SINGLE("customer_outstanding_balance_single", Role.SUPERVISOR),
    CUSTOMER_REFUNDS_SINGLE("customer_refunds_single", Role.SUPERVISOR),
    CUSTOMER_ACTIVITY_TIMELINE_SINGLE("customer_activity_timeline_single", Role.SUPERVISOR),

    CUSTOMER_ACTIVITY_SUMMARY("customer_activity_summary", Role.MANAGER),
    CUSTOMER_ACTIVITY_TIMELINE_ALL("customer_activity_timeline_all", Role.MANAGER),
    TOP_CUSTOMERS_BY_REVENUE("top_customers_by_revenue", Role.MANAGER),
    INACTIVE_CUSTOMERS("inactive_customers", Role.MANAGER),

    // ===================== ACCOUNTING =====================
    BALANCE_SHEET("balance_sheet", Role.ADMIN),
    CASH_FLOW_STATEMENT("cash_flow_statement", Role.ADMIN),
    ACCOUNTS_RECEIVABLE_SUMMARY("accounts_receivable_summary", Role.ADMIN),
    ACCOUNTS_PAYABLE_SUMMARY("accounts_payable_summary", Role.ADMIN),
    ...
    INVENTORY_ADJUSTMENTS_REPORT("inventory_adjustments_report", Role.MANAGER),
    INVENTORY_TURNOVER_REPORT("inventory_turnover_report", Role.MANAGER),

    // ===================== SUPPLIERS =====================
    SUPPLIER_LIST("supplier_list", Role.SUPERVISOR),
    SUPPLIER_ITEMS_SUPPLIED_SINGLE("supplier_items_supplied_single", Role.MANAGER),
    SUPPLIER_ITEMS_SUPPLIED_ALL("supplier_items_supplied_all", Role.MANAGER),
    SUPPLIER_PRICE_VARIANCE("supplier_price_variance", Role.ADMIN),
    SUPPLIER_PRICE_VARIANCE_BY_PRODUCT("supplier_price_variance_by_product", Role.ADMIN),

    // ===================== PAYMENTS =====================
    PAYMENTS_SUMMARY("payments_summary", Role.ADMIN),
    PAYMENTS_BY_METHOD("payments_by_method", Role.ADMIN),
    DAILY_PAYMENTS_REPORT("daily_payments_report", Role.SUPERVISOR),
    ...
    public static ReportAccess fromReportName(String reportName) {
        ...
        throw new IllegalArgumentException("No access rule defined for report: " + reportName);
    }
}