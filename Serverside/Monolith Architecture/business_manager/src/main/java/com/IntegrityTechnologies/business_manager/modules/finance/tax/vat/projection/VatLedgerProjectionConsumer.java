package com.IntegrityTechnologies.business_manager.modules.finance.tax.vat.projection;
...
public class VatLedgerProjectionConsumer {
    ...
    private void process(JournalPostedEvent event) {
        ...
        if (
                "VAT_CLEARING".equals(event.sourceModule()) ||
                "VAT_PAYMENT".equals(event.sourceModule()) ||
                "VAT_REFUND".equals(event.sourceModule())
        ) {
            return;
        }
        ...
        try {
            ...
            for (LedgerEntryDTO entry : event.entries()) {
                ...
                } else if (entry.accountId().equals(inputVat)) {
