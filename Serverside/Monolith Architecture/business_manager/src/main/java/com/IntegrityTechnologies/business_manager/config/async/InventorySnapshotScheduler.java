            ...
            switch (mode.toLowerCase()) {
                ...
                case "weekly" -> {
                    DayOfWeek dow = DayOfWeek.valueOf(
                            env.getProperty("inventory.snapshot.weekday", "MONDAY")
                    );
                    if (today.getDayOfWeek() == dow) {
                        inventoryService.takeSnapshot(today);
                    }
                }

                case "monthly" -> {
                    int day = Integer.parseInt(
                            env.getProperty("inventory.snapshot.monthday", "1")
                    );
                ...
                case "yearly" -> {
                    int dayOfYear = Integer.parseInt(
                            env.getProperty("inventory.snapshot.yearday", "1")
                    );
                    if (today.getDayOfYear() == dayOfYear) {
                        inventoryService.takeSnapshot(today);
                    }
                }
            }
